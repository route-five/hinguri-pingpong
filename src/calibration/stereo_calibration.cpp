...
int main() {
  cv::Mat camera_matrix_left, dist_coeffs_left,
          camera_matrix_right, dist_coeffs_right;

  cv::FileStorage fs_left(CameraType::LEFT.calibration_matrix_path(), cv::FileStorage::READ);
  fs_left["camera_matrix"] >> camera_matrix_left;
  fs_left["dist_coeffs"] >> dist_coeffs_left;
  fs_left.release();

  cv::FileStorage fs_right(CameraType::RIGHT.calibration_matrix_path(), cv::FileStorage::READ);
  ...
  const auto image_paths = glob_chessboard_images("img/chessboard/right");
