...
private:
    ...
    [[nodiscard]] static cv::Scalar_<int> color(const int R, const int G, const int B) {
    ...
    void render_pingpong_table() const {
        // 탁구대 그리기
        for (int i = 0; i < 4; ++i) {
            cv::line(img, to_pixel(vertices[i]), to_pixel(vertices[(i + 1) % 4]), color(0, 0, 0), 2);
        }

        // 중앙 네트 라인 그리기
        const cv::Vec2f mid_left = {0, TABLE_HEIGHT / 2};
        const cv::Vec2f mid_right = {TABLE_WIDTH, TABLE_HEIGHT / 2};
        cv::line(img, to_pixel(mid_left), to_pixel(mid_right), color(80, 80, 80), 1, cv::LINE_AA);
    }

    void render_axis(const int arrow_size = 40) {
        cv::arrowedLine(img, {border, img.rows - border}, {border + arrow_size, img.rows - border}, color(0, 0, 0),
                        2);
        cv::arrowedLine(img, {border, img.rows - border}, {border, img.rows - border - arrow_size}, color(0, 0, 0),
                        2);
    ...
    std::tuple<cv::Vec2f, cv::Vec2f, float> render_initial_state() {
        Random<float> random;
        ...
        // 방향 범위 경계선 그리기
        cv::line(img, to_pixel(init_pos), to_pixel(vec_left), color(0, 0, 255), 1, cv::LINE_AA);
        cv::line(img, to_pixel(init_pos), to_pixel(vec_right), color(0, 0, 255), 1, cv::LINE_AA);
        ...
        cv::arrowedLine(img, to_pixel(init_pos), to_pixel(end_pos), color(0, 0, 255), 2, cv::LINE_AA, 0, 0.02);
    ...
    void render_projected_trajectory(const cv::Vec2f &init_pos, const cv::Vec2f &end_pos, float angle) {
        ...
        cv::arrowedLine(img, to_pixel(projected_trajectory[0]), to_pixel(projected_trajectory[1]),
                        color(255, 0, 0), 2,
                        cv::LINE_AA, 0, 0.02);
        cv::line(img, to_pixel(projected_trajectory[0]), to_pixel(init_pos), color(255, 128, 0), 1, cv::LINE_AA);
        cv::line(img, to_pixel(projected_trajectory[1]), to_pixel(end_pos), color(255, 128, 0), 1, cv::LINE_AA);
    }
...
public:
    ...
    void render() {
        ...
        auto [init_pos, end_pos, angle] = render_initial_state();
        render_projected_trajectory(init_pos, end_pos, angle);
