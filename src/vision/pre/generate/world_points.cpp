...
int main() {
    ...
    while (x <= TABLE_WIDTH - TABLE_OUTLINE_SIZE - TABLE_GRID_SIZE) {
        float y = TABLE_OUTLINE_SIZE + TABLE_GRID_SIZE + TABLE_TAPE_SIZE / 2;

        while (y <= TABLE_HEIGHT - TABLE_OUTLINE_SIZE - TABLE_GRID_SIZE) {
    ...
    cv::FileStorage fs(PATH_WORLD_POINTS, cv::FileStorage::WRITE);
    fs << "points" << world_pts;
    fs.release();
