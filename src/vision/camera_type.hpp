...
public:
  ...
  static const CameraType TOP_LEFT;
  static const CameraType TOP_RIGHT;

  CameraType(const CameraType&) = default;
  CameraType& operator=(const CameraType&) = delete;

  ~CameraType() = default;

  [[nodiscard]] std::string get_calibration_image_dir() const {
    return "img/chessboard/" + data_source_ + "/";
  }

  [[nodiscard]] std::string get_calibration_matrix_path() const {
    return "data/calibration/" + data_source_ + ".yml";
  }

  [[nodiscard]] std::string get_camera_points_path() const {
    return "data/points/" + data_source_ + "_camera.yml";
  }

  [[nodiscard]] std::string get_projection_matrix_path() const {
    return "data/projection/" + data_source_ + ".yml";
  }
  ...
  bool read_calibration_matrix(cv::Mat* camera_matrix = nullptr, cv::Mat* dist_coeffs = nullptr, cv::Mat* rvecs = nullptr,
    ...
    cv::FileStorage fs(get_calibration_matrix_path(), cv::FileStorage::READ);
    if (!fs.isOpened())
      return false;
  ...
  bool write_calibration_matrix(const cv::Mat* camera_matrix = nullptr, const cv::Mat* dist_coeffs = nullptr,
    ...
    cv::FileStorage fs(get_calibration_matrix_path(), cv::FileStorage::WRITE);
    if (!fs.isOpened())
      return false;
  ...
  bool read_camera_points(std::vector<cv::Point2f>& points) const {
    cv::FileStorage fs(get_camera_points_path(), cv::FileStorage::READ);
    if (!fs.isOpened())
      return false;
  ...
  bool write_camera_points(const std::vector<cv::Point2f>& points) const {
    cv::FileStorage fs(get_camera_points_path(), cv::FileStorage::WRITE);
    if (!fs.isOpened())
      return false;
  ...
  bool read_projection_matrix(cv::Mat* P = nullptr, cv::Mat* R = nullptr, cv::Mat* t = nullptr) const {
    cv::FileStorage fs(get_projection_matrix_path(), cv::FileStorage::READ);
    if (!fs.isOpened())
      return false;
  ...
  bool write_projection_matrix(const cv::Mat* P = nullptr, const cv::Mat* R = nullptr, const cv::Mat* t = nullptr) const {
    cv::FileStorage fs(get_projection_matrix_path(), cv::FileStorage::WRITE);
    if (!fs.isOpened())
      return false;
  ...
  [[nodiscard]] std::string get_data_source() const {
    return data_source_;
  }

  [[nodiscard]] cv::Size get_resolution() const {
    return resolution_;
  }
...
private:
  ...
  const std::string data_source_;
  const cv::Size resolution_;

  explicit CameraType(const std::string& name, const cv::Size& resolution) : name_{name}, data_source_{name}, resolution_{resolution} {
  }

  explicit CameraType(const std::string& name, const std::string& data_source, const cv::Size& resolution) : name_{name},
    data_source_{data_source}, resolution_{resolution} {
  }
};
...
const CameraType CameraType::TOP_LEFT{"top_left", "top", {960, 540}};
const CameraType CameraType::TOP_RIGHT{"top_right", "right", {960, 540}};
...
public:
  StereoCameraType(const CameraType& type1 = CameraType::TOP_LEFT, const CameraType& type2 = CameraType::TOP_RIGHT)
  ...
  [[nodiscard]] std::string get_name() const {
    return camera_type1.get_name() + "_" + camera_type2.get_name();
  }

  [[nodiscard]] std::string get_calibration_matrix_path() const {
    return "data/stereo_calibration/" + get_name() + ".yml";
  }

  [[nodiscard]] std::string get_projection_matrix_path() const {
    return "data/stereo_projection/" + get_name() + ".yml";
  }
