...
public:
    ...
    void process_data(
        ...
        if (PREDICT_MIN_Y <= world_pos.y && world_pos.y <= PREDICT_MAX_Y) {
            if (const auto find_prediction = predictor.predict()) {
                const auto& [init_pos, init_speed, pred_pos, pred_speed, duration] = find_prediction.value();
    ...
    void run(
        ...
        while (true) {
            ...
            if (key == 'q') {
                ...
                cv::FileStorage fs(std::format("{}/{}.yml", PATH_ORBIT, save_count++), cv::FileStorage::WRITE);
                if (!fs.isOpened()) {
...
int main() {
    ...
    auto vision_thread_pnp = std::thread(
        &VisionEndPnP::run, &vision_end_pnp,
        std::ref(queue), std::ref(queue_mutex), std::ref(queue_push_flag), std::ref(stop), std::ref(execute_done)
    );
