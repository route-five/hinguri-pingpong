...
int main() {
  ...
  while (true) {
    ...
    const auto largest_contour = tracker.largest_contour(contours);
    ...
    if (largest_contour.has_value()) {
      auto moment = largest_contour->moment();
      radius = largest_contour->min_enclosing_circle().second;

      cv::circle(frame, moment, static_cast<int>(radius), COLOR_RED, -1,
                 cv::LINE_AA); // Red largest contour

      meas.at<float>(0) = moment.x;
      meas.at<float>(1) = moment.y;
      kf.correct(meas); // Correct the Kalman filter with the measurement
    }

    // Draw the corrected point from Kalman filter
    cv::Point corrected_pt(kf.statePost.at<float>(0),
                           kf.statePost.at<float>(1));
    cv::circle(frame, corrected_pt, static_cast<int>(radius), COLOR_GREEN, -1,
               cv::LINE_AA); // Green corrected point
    ...
    cv::imshow("Moment + Kalman filter", frame);
